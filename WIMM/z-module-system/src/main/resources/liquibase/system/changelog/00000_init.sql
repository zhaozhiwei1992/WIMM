...
COMMENT ON COLUMN sys_menu.menu_type IS '菜单类型（1菜单 2按钮）';
...
COMMENT ON COLUMN sys_permissions.code IS '权限编码';
COMMENT ON COLUMN sys_permissions.type IS '权限类型,1:数据权限';
...
-- 初始化序列: admin 用户固定 id=1, 后续自增从 2 开始
SELECT setval(pg_get_serial_sequence('sys_user','id'), 2, false);
...
-- 菜单/用户都用了固定 id 插入, 重置序列, 避免后续自增冲突
SELECT setval(pg_get_serial_sequence('sys_menu','id'), (SELECT COALESCE(MAX(id),0)+1 FROM sys_menu), false);
...
-- acct_account_cls 同样用了固定 id, 重置序列
